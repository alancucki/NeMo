...
class FastPitchModel(SpectrogramGenerator):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.fastpitch = FastPitchModule(input_fft, output_fft, duration_predictor, pitch_predictor,
                                         cfg.n_speakers, cfg.symbols_embedding_dim,
                                         cfg.pitch_embedding_kernel_size, cfg.n_mel_channels)
    ...
    def generate_spectrogram(self, tokens: 'torch.tensor', speaker: int = 0, pace: float = 1.0
    ) -> torch.tensor:
        self.eval()
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, mel_loss, dur_loss, pitch_loss = self.loss(
            spect_predicted=mels_pred, log_durs_predicted=log_durs_pred,
            pitch_predicted=pitch_pred, spect_tgt=mels, durs_tgt=durs,
            dur_lens=text_lens, pitch_tgt=pitch)
